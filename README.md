...
Deploy: `npm run build` writes HTML at the repo root; `npm run build:dist` also copies the public files into `dist/` for Cloudflare Workers. GitHub Actions still publishes `dist/` to GitHub Pages. Cloudflare preview is `npm run deploy` (Wrangler). Connect the GitHub repo in Workers Builds when ready (`build:dist`, then `npx wrangler deploy`).
...
**GitHub Pages:** still live on the custom domain. **Cloudflare:** `npm run deploy` publishes `dist/` to the `felipebasurto-com` Worker (`*.workers.dev`). Do not attach `felipebasurto.com` until that preview looks right.
